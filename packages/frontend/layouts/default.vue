  ...
  <NaiveConfig :theme-config="themeConfig">
    <NLayout has-sider position="absolute">
      ...
      <NLayoutSider bordered show-trigger collapse-mode="width" :collapsed-width="64" :width="240"
        :native-scrollbar="false" style="max-height: 320px" v-model:collapsed="collapsed">
        <div class="flex justify-center">
          <div>
            <NImage src="/logo.png" class="h-9 mt-4 mb-4" previewDisabled />
          </div>
          <div v-if="!collapsed" class="pt-0 mt-5 ml-4 ">
            <NH2 class="text-primary font-semibold">TEAMSCODE</NH2>
          </div>
        </div>
        <NMenu :collapsed-width="64" :collapsed-icon-size="22" :options="menuOptions" v-model:value="activeMenu" />
      </NLayoutSider>
      <NLayout class="flex h-min-screen flex-col">
        <NLayoutHeader bordered class="px-6 py-3 flex">
          ...
          <NuxtLink to="/auth/register">
            <NButton class="ml-auto" type="primary">Sign up
              <template #icon>
                <NIcon>
                  <SignupIcon />
                </NIcon>
...
import { HomeOutlined as HomeIcon, UserAddOutlined as SignupIcon, TrophyOutlined as ContestIcon, LoginOutlined as LoginIcon } from '@vicons/antd'
