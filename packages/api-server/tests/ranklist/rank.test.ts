import { loadTestingApp, FastifyTypeBox } from '../util/app.js'
...
import {TeamScore, UserRole} from '@argoncs/types'
...
tap.test(
  ...
  async t => {
    ...
    const addPoints = async (teamId: string, pts: number) => {
      const score = (await teamScoreCollection.findOne({ id: teamId }))!.totalScore;
